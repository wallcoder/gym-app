...
onMounted(async () => {
  ...
  if (userFromStorage) {
    currentUser.value = JSON.parse(userFromStorage); // Parse user data and store it in currentUser ref
  }
});
...
<template>
  <header
    ...
    <RouterLink to="/">
      <img :src="logoChange" alt="logo" class="w-[50px]">
    </RouterLink>
    <nav class="hidden laptop:flex laptop:items-center">
      ...
      <div class="relative ml-16 cursor-pointer" v-if="isLogin" @click="toggleDropdown">
        ...
        <div v-if="isDropdownOpen"
          class="absolute right-0 text-[1.05rem] top-14 text-black mt-0 w-35 bg-white rounded-lg shadow-lg text-md">
          <ul>
            <li class="py-2 px-2 hover:bg-first hover:text-white cursor-pointer rounded-t-lg">Profile</li>
            <li class="py-2 px-2 hover:bg-first hover:text-white cursor-pointer">Notifications</li>
            <li class="py-2 px-2 hover:bg-first hover:text-white cursor-pointer">Settings</li>
            <li class="py-2 px-2 hover:bg-first hover:text-white cursor-pointer">My Gym</li>
            <li class="py-2 px-2 hover:bg-first hover:text-white cursor-pointer">Membership</li>
            <li class="py-2 px-2 hover:bg-first hover:text-white cursor-pointer">Saved</li>
            <li class="py-2 px-2 hover:bg-first hover:text-white cursor-pointer rounded-b-lg" @click="handleLogout">Logout</li>
          </ul>
        </div>
      </div>
    </nav>
