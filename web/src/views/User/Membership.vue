...
<template>
    <section>
        ...
        <div class="p-2 flex flex-col w-full rounded-lg height-full">
            ...
            <div class="w-full h-70 flex justify-center items-center font-semibold text-sixth text-3xl  "
                v-if="userPlans.length === 0">No Membership Plans</div>
            <div v-else class="grid tablet:grid-cols-2 gap-4">
                <div class="flex space-x-2 p-4 shadow-3 rounded-lg text-sm justify-between" v-for="p in userPlans" :key="p.id" >
                    <div class="flex flex-col justify-center">
                        ...
                        <h2 class="text-first hover:underline font-semibold" v-if="p.status == 'inactive'">Renew Membership
                        </h2>
                    </div>
