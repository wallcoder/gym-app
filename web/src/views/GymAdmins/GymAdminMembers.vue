...
const { gymIdRef, items } = storeToRefs(gymAdmin)
const { getAllGymMembers } = gymAdmin
...
onMounted(() => {
    getAllGymMembers(gymIdRef.value)
    loading.value = false
})

watch(search, async (newVal) => {
    await getAllGymMembers(newVal);
})
...
<template>
    <section class="flex flex-col">
        ...
        <div
            ...
            <h1 class="text-xl mb-2">Members</h1>
            <div class="flex justify-between">
                ...
                <span>
                    <span></span>
                    <!-- <i class="fa-solid fa-plus cursor-pointer text-lg"></i> -->
            ...
            <div v-else>
                ...
                <div class="overflow-auto rounded-lg shadow hidden md:block">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr class="" v-for="i in items.allItems" :key="i.id">
                                ...
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">{{ i.email }}</td>
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    <span
                                        class="p-1.5 text-xs font-medium uppercase tracking-wider text-grey  rounded-lg bg-opacity-50"
                                        :class="i.PlanMappings[0].status == 'active' ? 'bg-green-400' : 'bg-orange-600'">{{
                                            i.PlanMappings[0].status }}</span>
                                </td>
                                <td class="p-3 text-sm text-gray-700 whitespace-nowrap uppercase">{{ i.UserRole.roleName }}
                                </td>
                                <!-- <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                <span
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
                    <div class=" space-y-3 p-4 rounded-lg shadow" v-for="i in items.allItems" :key="i.key">
                        <div class="flex items-center space-x-2 text-sm">
                            <div>
                                <a href="#" class="text-blue-500 font-bold hover:underline">#{{ i.id }}</a>
                            </div>
                            <div class="text-gray-500">{{ i.firstName }} {{ i.lastName }}</div>
                            <div>
                                <span
