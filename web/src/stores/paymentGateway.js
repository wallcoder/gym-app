...
export const usePaymentGatewayStore = defineStore('paymentGateway', () => {
    ...
    const { updatePlanMapping, createPlanMapping } = planStore
    ...
    const payNowRecharge = async (amountValue, gymId = null, planId, planMapId, userId = '', transactionType, currency = 'INR', receipt = 'receipt#1') => {
        const token = localStorage.getItem('token');
        if (!token) {
        ...
        const options = {
            ...
            handler: async (response) => {
                ...
                try {
                    ...
                    await storePaymentDetails(response.razorpay_payment_id, userId, planId, transactionType, amountValue, 'success', gymId, verificationResponse.data, paymentType);
                    await updatePlanMapping(planMapId)
                } catch (error) {
                    console.error('Error verifying payment:', error);
    ...
    return { payNowRecharge, orderId, createOrder, storePaymentDetails, payNow, fetchTestKey };
});
