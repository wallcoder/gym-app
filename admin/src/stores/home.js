...
export const useHomeStore = defineStore('home', () => {
    ...
    watch(password, (newPassword) => {
        ...
        if (passwordRegex.test(newPassword) || newPassword.trim() === '') {
            ...
            message.value.password = "Password not strong enough"
        }
    ...
    const handleLogin = async (roleId=2) => {
        try {
            console.log(email.value, password.value);
            const response = await axios.post('/login', { email: email.value, password: password.value, roleId: roleId});
            const token = response.data.token;
    ...
    return {
        handleLogoutAdmin, isOpenRate, currPassword, isOpenChangePassword, isOpenDeleteAccount, callback, closeOtp, modal, delUserByEmail, isOpenLogin, isOpenRegisterGym, isOpenSignUp, closeModals, isOpenOTP, firstName, lastName, email, password, conPassword, message, handleRegister, handleSave, getSaved, savedGyms,
